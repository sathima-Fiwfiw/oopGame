...
public class GameClient {
        ...
        panel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                for (int i = 0; i < maxCandiesToDraw; i++) {
                    System.out.println("Number of candies: " + candyPositions.size());
    ...
    private void sendCandyPosition() {
        StringBuilder sb = new StringBuilder("candy");
        for (Point candyPos : candyPositions) {
            sb.append(",").append(candyPos.x).append(",").append(candyPos.y);
        }
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()))) {
            ...
            while ((message = in.readLine()) != null) {
                ...
                if (parts.length == 4) {
                    ...
                    //candyPositions.clear(); // Clear existing candy positions
                    // Update candy positions
                    for (int i = 1; i < parts.length; i += 2) {
                        int candyX = Integer.parseInt(parts[i]);
                        int candyY = Integer.parseInt(parts[i + 1]);
                        candyPositions.add(new Point(candyX, candyY)); // Add new candy position
                    }
                    panel.repaint(); // Refresh the display after updating candy positions
